...
func (f *fakeRepo) GetMessageByID(ctx context.Context, userID, provider, emailMessageID string) (*models.EmailMessage, error) {
	return nil, nil
}
...
func TestGmailProvider_FetchSummaries(t *testing.T) {
	repo := &fakeRepo{}
	svc := NewMessageService(repo, nil, WithGmailAPI(&mockGmailAPI{}))
	provider := NewProvider(svc)
	ctx := context.Background()
	userUUID := uuid.MustParse("11111111-1111-1111-1111-111111111111")
	userID := userUUID.String()
	ctx = ContextWithUserID(ctx, userID)
	ctx = context.WithValue(ctx, common.OAuthTokenKey{}, &oauth2.Token{AccessToken: "test-token"})
	ctx = context.WithValue(ctx, gmailParamsKey{}, &FetchParams{Limit: 10})

	summaries, err := provider.FetchSummaries(ctx, userID)
	if err != nil {
	...
	svcErr := NewMessageService(errRepo, nil, WithGmailAPI(&mockGmailAPI{}))
	providerErr := NewProvider(svcErr)
	_, err = providerErr.FetchSummaries(ctx, userID)
	if err == nil {
...
func (f *fakeRepoWithError) GetMessageByID(ctx context.Context, userID, provider, emailMessageID string) (*models.EmailMessage, error) {
	return nil, nil
}
...
func (f *fakeRepoForFetch) GetMessageByID(ctx context.Context, userID, provider, emailMessageID string) (*models.EmailMessage, error) {
	return f.msg, f.err
}
