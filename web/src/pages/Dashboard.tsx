    ...
    >
      ...
      <nav className="w-full flex items-center justify-between px-4 py-3 bg-transparent z-10">
        ...
        <div className="hidden md:flex items-center gap-8">
          <Link to="/" className="text-base font-normal text-gray-300 hover:text-accent transition">Home</Link>
          <Link to="/dashboard" className="text-base font-bold text-accent">Dashboard</Link>
        </div>
      </nav>
      {/* DASHBOARD HEADER */}
      <section className="flex flex-col items-center justify-center pt-8 pb-2 px-4 z-10 relative">
        ...
        <p className="text-lg text-gray-300 mb-6">Here’s a preview of your inbox, prioritized by AI.</p>
      </section>
      {/* INBOX PREVIEW */}
        ...
        <div className="w-full max-w-2xl bg-[#181f2a] bg-opacity-90 rounded-2xl shadow-2xl border border-gray-800 p-6">
          ...
          <ul className="divide-y divide-gray-800">
            {MOCK_EMAILS.map(email => (
              <li key={email.id} className="py-4 flex flex-col md:flex-row md:items-center md:justify-between group hover:bg-[#23243a] rounded-lg px-2 transition">
                <div>
                  <div className="font-semibold text-white group-hover:text-accent transition text-lg">{email.subject}</div>
